...
# prepare_presentation_data.R (grand-average voltage per
# channel per timepoint, all 64 channels, dots task only, the same
# collapse-over-subject-and-trial approach as aug26_1.R's
...
if (is.null(grand_average_erp)) {
  ggplot() +
    annotate(
      ...
      size = 5,
      colour = accent,
      label = "tmp/grand_average_erp.rds not found.\nRun prepare_presentation_data.R in the devcontainer."
    ) +
    theme_void()
} else {
  grand_average_erp |>
...
# the pooled grand average: prepare_presentation_data.R writes
...
# one participant only: prepare_presentation_data.R writes this
...
if (is.null(m0_fitted)) {
  ggplot() +
    annotate(
      ...
      size = 5,
      colour = accent,
      label = "tmp/m0_fitted.rds not found.\nRun prepare_presentation_models.R."
    ) +
    theme_void()
} else {
  ggplot() +
...
if (is.null(grand_average_poz) || is.null(m2_population)) {
  ggplot() +
    annotate(
      ...
      size = 5,
      colour = accent,
      label = "Data not found.\nRun prepare_presentation_data.R and prepare_presentation_models.R."
    ) +
    theme_void()
} else {
  ggplot() +
